...
def BLAST():
    global input_file
    global output_file
    blast_results = NcbiblastnCommandline(query="%s" %input_file, db="/biol/programs/ncbiBLAST/db/nt", outfmt='"6 qseqid stitle evalue"', out="%s" %output_file, negative_gilist="%s/Invalid_GIs.gi" %path, word_size=11, num_alignments=10, max_hsps=1, num_threads=70 )
      ...
      if number_of_contigs>1:
       ...
       while Evalue==True:
            ...
            #BLAST longest contig
                blast_results = NcbiblastnCommandline(query="BiggestContig.txt", db="/biol/programs/ncbiBLAST/db/nt", outfmt='"6 qseqid stitle evalue"', out="%s_LargestContigBlastN.txt" %clustername, negative_gilist="%s/Invalid_GIs.gi" %path, word_size=11, num_alignments=1, max_hsps=1, num_threads=60 )
                stdout, stderr = blast_results()
                if stdout != '' or stderr != '':
                    print("BLAST returned unexpected output")
                    print(stdout, stderr)
